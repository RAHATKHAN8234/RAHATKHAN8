...
module.exports.run = function({
    ...
    if (/https:\/\/(\w+\.)?tiktok+\.com\//.test(args[1])) return get(`${CN}/tiktok/download?link=${args[1]}`).then(async response_api => {
        if (args[0] == 'video') {
            const msg_info = `${infoVideo(response_api.data)}\n\n[⚜️]➜ Reply [ sd | wm | hd ] to download video🥰credits+Khan Rahul RK`;
    ...
    if (args[0] == 'audio') {
        ...
        if (args[0] == 'info') return get(`${CN}/tiktok?username=${args[1]}`).then(async response_api => JSON.stringify(response_api.data.userInfo) == '{}' ? api.sendMessage(`[⚜️]➜ No user results found ${args[1]}`, event.threadID, event.messageID): api.sendMessage({body: infoUser(response_api.data), attachment: await downLoad(response_api.data.userInfo.user.avatarLarger, `${__dirname}/cache/tiktok_info_user_${event.messageID}.jpg`)}, event.threadID, () => unlinkSync(`${__dirname}/cache/tiktok_info_user_${event.messageID}.jpg`), event.messageID)).catch(e => api.sendMessage(e, event.threadID, event.messageID));
...
module.exports.handleReply = async function({ handleReply: $, api, event }){
    ...
    switch ($.case) {
        case 'download-video': {
            if (!['sd', 'wm', 'hd'].includes(lower1)) return api.sendMessage(`[⚜️]➜ Malformed !.`, event.threadID, event.messageID);
         ...
         case 'trending': {
           if (lower1 == 'trang'){
               if (isFinite(lower2) && lower2 <= roof($.data.data.length/6)) /* yeuTrang */ return runInfoTrending($.data, api, event, this.config.name, 6, +lower2, $.menu); else return api.sendMessage(`[⚜️]➜ Page not found ${lower2} in the list`, event.threadID, event.messageID);
           };
           if (isFinite(lower1) && !!lower2 && !['wm'].includes(lower2)) return api.sendMessage(`[⚜️]➜ Malformed !.`, event.threadID, event.messageID);
         ...
         case 'search': {
             if (lower1 == 'trang'){
               if (isFinite(lower2) && lower2 <= roof($.data.length/6)) /* yeuTrang */ return runInfoSearch($.data, api, event, this.config.name, 6, +lower2, $.menu); else return api.sendMessage(`[⚜️]➜ Page not found ${lower2} in the list`, event.threadID, event.messageID);
           };
           if (isFinite(lower1)) {
...
async function runInfoTrending(a, b, c, d, e, g, h) {
    const dirTD = `${__dirname}/cache/tiktok_trending_${c.senderID}_${c.messageID}`;
           ...
           if (h == 'audio') {
            ...
            txt += `[⚜️]➜ Trang [${g}/${roof(a.data.length/e)}]\n[⚜️]➜ Reply [STT] to download audio.\n[⚜️]➜ Reply [page number] to switch tabs.\n[⚜️]➜ [Reaction] to cycle through the video list.`;
           } else {
               for (var i = (e*g)-e; i < e*g; i++) {
                if (!a.data || !a.data[i]) break;
            ...
            txt += `[⚜️]➜ Trang [${g}/${roof(a.data.length/e)}]\n[⚜️]➜ Reply [STT | STT wm] to download the video.\n[⚜️]➜ Reply [page number] to switch tabs.\n[⚜️]➜ [Reaction] to cycle through the audio list.`;
           };
...
async function runInfoSearch(a, b, c, d, e, g, h){
    ...
    txt += `[⚜️]➜ Trang [${g}/${roof(a.length/e)}]\n[⚜️]➜ Reply [STT] to download Video.\n[⚜️]➜ Reply [page number] to switch tabs.\n[⚜️]➜ [Reaction] to cycle through audio list.`;
    };
    await b.sendMessage({body: txt, attachment}, c.threadID, (y, z) => {
                const option = {
                    name: d,
