...
const d1 = args[1]?args[1].toLowerCase() : '' ;
 if (!d1 || !validCommands.includes(d1))return;
  ...
  query2 = query;} else {
        ...
        const svRes = await axios.get(`${global.config.api}/album?add=${query2}&url=${imgurLink}`);
...
module.exports.handleReply = async function ({ api, event, handleReply }) {
        const admin = "YOUR UID";
        api.unsendMessage(handleReply.messageID);
        if (event.type == "message_reply") {
        const reply = parseInt(event.body);
        if (isNaN(reply)) {
    return api.sendMessage("🔰 | Please reply with either 1 - 20", event.threadID, event.messageID);
  }
  let query;
  let cp;
  if (reply === 1) {
    ...
    cp = "𝗡𝗮𝘄 𝗕𝗮𝗯𝘆 𝗖𝗿𝗶𝗰𝗸𝗲𝘁 𝘃𝗶𝗱𝗲𝗼<😙";
      ...
      query = "random";
    cp = "𝗡𝗮𝘄 𝗕𝗮𝗯𝘆 𝗥𝗮𝗻𝗱𝗼𝗺 𝘃𝗶𝗱𝗲𝗼<😙";
    }
    else if (reply === 22) {
      query = "car";
    cp = "𝗡𝗮𝘄 𝗕𝗮𝗯𝘆 𝗖𝗮𝗿 𝘃𝗶𝗱𝗲𝗼<🚗";
    }
    else if (reply === 23) {
      query = "bike";
    cp = "𝗡𝗮𝘄 𝗕𝗮𝗯𝘆 𝗕𝗶𝗸𝗲 𝘃𝗶𝗱𝗲𝗼<🏍";
    } 
    else {
      api.sendMessage("vag luccha",event.threadID,event.messageID)
      return
          }
  try {
